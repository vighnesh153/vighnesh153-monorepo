...
import {
  ...
  isGistPublic,
} from "../githubGistOptionsUtils.ts";
...
describe('Helpers > "getEnableRequestCaching" tests', () => {
  ...
  it("should return the provided value in options", () => {
    ...
    expect(
      getEnableRequestCaching({ ...getBaseConfig(), enableRequestCaching }),
    ).toBe(enableRequestCaching);
  });
});

describe('Helpers > "getCorsConfig" tests', () => {
  it("should return the default value if options.getCorsConfig is undefined", () => {
    expect(getCorsConfig({ ...getBaseConfig() })).toMatchObject({
      type: "default",
    });
  });
  ...
  it("should return the provided value in options", () => {
    ...
    expect(getCorsConfig({ ...getBaseConfig(), corsConfig: randomCorsConfig }))
      .toMatchObject(randomCorsConfig);
  });
});
