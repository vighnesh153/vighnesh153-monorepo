...
export function getDistantCorner(
  point: Point,
  repelPoint1: Point,
  repelPoint2: Point,
): Point {
  const distance =
    euclidianDistance([point.x, point.y], [repelPoint1.x, repelPoint1.y]) / 4;
  ...
  const farthestPointsFromRepelPoint1 = findFarthestPoints(
    repelPoint1,
    potentialPoints,
  );
  const farthestPointsFromRepelPoint2 = findFarthestPoints(
    repelPoint2,
    potentialPoints,
  );

  const intersection = findIntersectionPoints(
    farthestPointsFromRepelPoint1,
    farthestPointsFromRepelPoint2,
  );
...
function findFarthestPoints(pivotPoint: Point, points: Point[]): Point[] {
  ...
  for (const point of points) {
    const dist = euclidianDistance([point.x, point.y], [
      pivotPoint.x,
      pivotPoint.y,
    ]);
    if (dist > maxDistance) {
