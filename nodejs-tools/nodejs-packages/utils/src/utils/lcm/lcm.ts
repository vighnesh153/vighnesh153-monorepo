...
const mergePrimeFactors = (
  ...
  const combinedPrimes = Array.from(new Set([...Object.keys(primeFactorsCount1), ...Object.keys(primeFactorsCount2)]));
  combinedPrimes.forEach((primeAsString) => {
    const prime = parseInt(primeAsString, 10);
    mergedPrimeFactors[prime] = Math.max(primeFactorsCount1[prime] ?? 0, primeFactorsCount2[prime] ?? 0);
  });
...
export function lcm(...numbers: number[]) {
  ...
  numbers.forEach((n) => {
    lcmPrimeFactorsCount = mergePrimeFactors(lcmPrimeFactorsCount, primeFactorize(n));
  });
