...
describe(
  ...
  () => {
    ...
    const defineVariable = (name: string, value: unknown, type: string) => {
      const variableBlock = new VariableBlock(VariableBlockType.declare, name, type, value, false, scope);
...
describe('should assign values to array or ' + 'modify some of them or throw if error.', () => {
  let blockParser: BlockParser;
  let scope: Scope;
  let linesOfCode: LineOfCode[];
  beforeEach(() => {
  ...
  const defineArray = (name: string, value: unknown, type: string) => {
    const variableBlock = new ArrayVariableBlock(VariableBlockType.declare, name, 'array', value, false, scope, type);
    variableBlock.execute();
  };
