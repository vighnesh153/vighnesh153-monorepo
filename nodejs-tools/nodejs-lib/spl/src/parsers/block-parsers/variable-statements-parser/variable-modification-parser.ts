...
export class VariableModificationParser extends BlockParser {
  ...
  private modifyingArray(identifier: string, value: string): Block {
    ...
    if (regexMatchResult) {
      ...
      if (arrayName === '') {
        throw new Error('Invalid array name at line: ' + this.lineOfCodes[this.lineOfCodes.length - 1].number);
      }
      // eslint-disable-next-line prefer-destructuring
      ...
      try {
        ...
        throw new Error(
          'Invalid index expression of array at line: ' + this.lineOfCodes[this.lineOfCodes.length - 1].number
        );
      }

      if (this.scope.hasVariable(arrayName) === false) {
        throw new Error(
          `Array: ${arrayName} not found at line: ` + this.lineOfCodes[this.lineOfCodes.length - 1].number
        );
      }
      ...
      if (array.type !== 'array') {
        throw new Error(
          `Variable: ${arrayName} is not an array at line: ` + this.lineOfCodes[this.lineOfCodes.length - 1].number
        );
      }
      ...
      if (index < 0 || index >= array.value.length) {
        throw new Error(
          `Index out of bounds exception at line: ` + this.lineOfCodes[this.lineOfCodes.length - 1].number
        );
      }
      ...
      if (type === 'NULL') {
        throw new Error(
          `Invalid value being assigned at line: ` + this.lineOfCodes[this.lineOfCodes.length - 1].number
        );
      }

      if (type !== array.arrayType) {
        throw new Error(`Inconsistent types at line: ` + this.lineOfCodes[this.lineOfCodes.length - 1].number);
      }
      ...
      return new VariableBlock(VariableBlockType.set, arrayName, array.type, newValues, false, this.scope);
    }
    throw new Error('Invalid modification of array at line: ' + this.lineOfCodes[this.lineOfCodes.length - 1].number);
  }
  ...
  parse(): Block {
    if (this.tryParse()) {
      ...
      if (result) {
        ...
        if (this.scope.hasVariable(variableName.trim()) === false) {
          throw new Error('Invalid identifier: ' + variableName.trim() + ' at line: ' + lineUnderTest.number);
        }
    ...
    throw new Error(
      'Invalid variable modification statement at line: ' + this.lineOfCodes[this.lineOfCodes.length - 1].number
    );
  }
}
