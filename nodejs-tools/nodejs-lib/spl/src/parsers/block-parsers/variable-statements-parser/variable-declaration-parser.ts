...
export class VariableDeclarationParser extends BlockParser {
  ...
  constructor(
    ...
    const scopeForEvaluation = evaluationScope
      ? (evaluationScope as Scope)
      : scope;
    ...
    this.expressionEvaluators.array = new ArrayExpressionEvaluator(
      scopeForEvaluation,
    );
  }
  ...
  parsePrimitive(): Block {
    ...
    if (result) {
      ...
      if (valueEvaluator.tryEvaluate(value) === false) {
        throw new Error(
          "Invalid value assigned to variable at line: " + lineUnderTest.number,
        );
      }
  ...
  parseArray(): Block {
    ...
    if (result) {
      ...
      const valueEvaluator = this.expressionEvaluators
        .array as ArrayExpressionEvaluator;
      ...
      if (valueType !== arrayType && valueType !== "any") {
        throw new Error(
          `Datatype of variable doesn't match ` +
  ...
  parse(): Block {
    ...
    throw new Error(
      "Invalid variable declaration statement at line: " +
