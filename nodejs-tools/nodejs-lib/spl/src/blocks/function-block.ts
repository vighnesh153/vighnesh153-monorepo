...
export class FunctionBlock extends Block {
  ...
  execute(): void {
    if (this.functionArguments.length !== this.argv.length) {
      throw new Error(`Expected ${this.functionArguments.length} arguments ` + `but got ${this.argv.length} instead.`);
    }
    ...
    for (let i = 0; i < this.functionArguments.length; i++) {
      try {
        ...
        throw new Error(
          `Invalid argument declaration or assignment: ` + `${this.functionArguments[i]} <= ${this.argv[i]}.`
        );
      }
    }
