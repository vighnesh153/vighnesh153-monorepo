...
import {
  parseCodeOperators,
  ParseCodeOperatorsOptions,
} from "./parseCodeOperators.ts";
import {
  parseCodeReservedWords,
  ParseCodeReservedWordsOptions,
} from "./parseCodeReservedWords.ts";
import {
  parseCodeStrings,
  ParseCodeStringsOptions,
} from "./parseCodeStrings.ts";
...
function codeSnippetsToHtml(codeSnippets: CodeSnippet[]): string {
  return codeSnippets
    .map((snippet) =>
      snippet.type === "code"
        ? snippet.value
        : `<span class="code-${snippet.type}">${snippet.value}</span>`
    )
    .join("");
}
...
export function simpleCodeToHtml(
  ...
  let codeSnippets: CodeSnippet[] = parseCodeStrings(code, {
    ...
    type: item.isString ? "string" : "code",
    value: item.value,
  }));
  ...
  codeSnippets = codeSnippets.reduce((prev, snippet) => {
    if (snippet.type === "code") {
      ...
      prev.push(
        ...res.map((item): CodeSnippet => ({
          type: item.isComment ? "comment" : "code",
          value: item.value,
        })),
      );
    } else {
  ...
  codeSnippets = codeSnippets.reduce((prev, snippet) => {
    if (snippet.type === "code") {
      ...
      prev.push(
        ...res.map((item): CodeSnippet => ({
          type: item.isOperator ? "operator" : "code",
          value: item.value,
        })),
      );
    } else {
  ...
  codeSnippets = codeSnippets.reduce((prev, snippet) => {
    if (snippet.type === "code") {
      ...
      prev.push(
        ...res.map((item): CodeSnippet => ({
          type: item.isReservedWord ? "reserved-word" : "code",
          value: item.value,
        })),
      );
    } else {
