...
const domainName = 'draw.vighnesh153.com';
...
export function WebsiteStack({ stack }: StackContext) {
  ...
  const hostedZone = HostedZone.fromLookup(stack, 'DrawVighnesh153DotCom_HostedZone', {
    domainName: hostedZoneDomainName,
  });

  const certificate = new Certificate(stack, 'DrawVighnesh153DotCom_ACM_Cert', {
    domainName,
    validation: CertificateValidation.fromDns(hostedZone),
  });

  const site = new StaticSite(stack, 'DrawVighnesh153DotCom', {
    path: '.',
    ...
    cdk: {
      bucket: {
        bucketName: `draw-vighnesh153-com-${stage}`,
      },
      distribution: {
        comment: `draw.vighnesh153.com`,
      },
    },
  });
