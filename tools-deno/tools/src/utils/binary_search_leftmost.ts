import {
  type BinarySearchComparator,
  defaultBinarySearchComparator,
} from "./binary_search.ts";

/**
 * Search for an item in an ascending sorted array (with duplicates) and return its left-most index.
...
export function binarySearchLeftmost<T>(
  ...
  while (startIndex <= endIndex) {
    ...
    // Element found
    if (compareResult === 0) {
      resultIndex = Math.min(resultIndex ?? Infinity, midIndex);
      endIndex = midIndex - 1;
    }
